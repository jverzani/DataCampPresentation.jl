...
Tuples allow for heterogeneous containers
...
Many objects are iterable (as the range `1:3` above). There are several *helper* functions for iteration. Among others, these examples show 3 ways to iterate over a matrix:
...
For performance reasons, `!` is used in many examples, but `:` is safer to use and consistent with the usage with matrix indexing.

For assignment, a distinction is important: using `:` the assignment is in place, which
means the type of the column can't change, whereas using `!` (or `s.variable`) it can. (This may be important, say,  if a column is read in as integer, but a floating point value is to be inserted, or if a coded value is replaced with `missing`.)
...
1229,1,UGRD,36923,D001,MTH,105,LEC,163486,"Carol,Carol",F,555-1212,carol.carol@euphoria.edu,,,,24007235,Frank,M,10:10:00.000000AM,12:05:00.000000PM,8/17/22,23247055,GRD
1229,1,UGRD,34534,D001,ENG,110,LEC,163486,"Carol,Carol",F,555-1212,carol.carol@euphoria.edu,,,,43993434,Faythe,W,10:10:00.000000AM,11:00:00.000000AM,8/17/22,23247055,GRD
1229,1,UGRD,43244,D011,PSY,100,LEC,185109,"Brock,Erin",U,555-2121,Brock.erin@euphoria.edu,,,,5435352,Grace,Th,,,,,
1229,1,UGRD,36923,D001,MTH,105,LEC,185109,"Brock,Erin",U,555-2121,Brock.erin@euphoria.edu,,,,24007235,Frank,M,,,,,
1229,1,UGRD,44332,D200,ENG,111,LEC,659056,"Mallory,Yves",M,555-2211,mallory.eve@euphoria.edu,,,,75544555,Mike,W,,,,,
"""
```
...
The `vcat` function combines vertically (there is also `hcat` and `hvcat`). We call `unique` as the `f22` data is arranged to have replicated data for each day a class meets.

```{julia}
s22 = vcat(s11[:,nms], unique(f22[:,nms]))
```
...
As aside this could also be done with an associative array:


```{julia}
grade_map = Dict("A"=>4.0, "A-"=>3.7, "B+"=>3.3, "B"=>3.0,
                 "B-"=>2.7, "C+"=>2.3, "C"=>2.0, "D"=>1.0,"F"=>0.0)

grade_to_number_alt(x) = get.(Ref(grade_map), x, missing)
```

The `gpa`  would just be done by applying `mean` (from the `Statistics` package), but we have to be a bit careful with
...
We use the `groupby` function to split, call the above on each student, and then combine into a data frame with this next call:


```{julia}
students = groupby(s22, :ID)
```
...
The middle one is a function, which may be an anonymous function, in which case parentheses may needed due to operator precedence.)
...
```{julia}
combine(students, :Term => minimum => :F,
        :Term => maximum => :L,
        :Term => length => :N,
        :Grade => gpa => :gpa)
```
...
* `>=(1209)` is a curried from of `(x,y) -> x >= y` with `y=1209` -- there are a few such operators for convenience with such tasks;
...
```{julia}
combine(groupby(d, :semesters), nrow => :N) |> df -> sort(df, :semesters)
```
...
This example is to create an alternate display for the table above.
...
```{julia}
combine(groupby(d, [:semesters, :F]), nrow => :N)
```
...
Students on the lower diagonal are still enrolled, other students have left. There are big variations between students who started in the fall versus the spring (more students start in the fall semester at Euphoria State). Here we select fall cohorts:

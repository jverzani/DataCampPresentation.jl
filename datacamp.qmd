...
The difference is that `:` makes a copy, whereas `!` makes a view.

For this presentation, it doesn't really matter, but for performance reasons, the `!` is commonly seen in examples and should be utilized when possible (though `:` is more idiomatic within `Julia`, it being used similarly with matrix or array indexing).
